...
Begin {
    ...
    Function Test-LocalLogging {
        $Path = Get-XMLConfigLoggingLocalPath
    ...
    Function Out-LogFile {
        ...
        if ($Mode -like "Local") {
            Test-LocalLogging
            $Logfile = "$LogLocal\ClientHealth.log"
        } else {
        ...
        Write-TextToLog -msg $text -type 1 -logpath $logFile
    }
    Function Write-TextToLog
    { 
        Param( [parameter(Mandatory=$false)][string]$msg = "",
               [parameter(Mandatory=$true)][ValidateRange(1,3)][int]$type,
               [parameter(Mandatory=$false)][string]$logpath = "")
        
        if ($ScriptLogFilePath -eq $null) {
            $global:ScriptLogFilePath = ""
        }
    
        $component = "$(Split-path $myInvocation.ScriptName -leaf):$($MyInvocation.ScriptLineNumber)"
        $file = (gci $myInvocation.ScriptName).BaseName
        $templogfile = "$LogLocal\$($file).log"
    
        If ($logpath.Length -gt 0) {
            $global:ScriptLogFilePath = $logpath
        } elseif ($ScriptLogFilePath.Length -eq 0) {
            $global:ScriptLogFilePath = $templogfile
        }
        
        try {
            if (!(Test-Path $ScriptLogFilePath -ErrorAction Stop)) {
                New-Item $ScriptLogFilePath -Type File -Force -ErrorAction Stop
            }
        } catch {
            $ScriptLogFilePath = $templogfile
            if (!(Test-Path $ScriptLogFilePath)) {
                New-Item $ScriptLogFilePath -Type File -Force | Out-Null
            }
        }
        ...
        $text = "<![LOG[$($msg)]LOG]!><time=`"$(Get-Date -Format HH:mm:ss.fff)$($UTC)`" date=`"$(Get-Date -Format M-d-yyyy)`" component=`"$($component)`" context=`"$([System.Security.Principal.WindowsIdentity]::GetCurrent().Name)`" type=`"$($type)`" thread=`"$([Threading.Thread]::CurrentThread.ManagedThreadId)`" file=`"$($file)`">"
    
        # APPEND the line to log file 
        Add-Content -Path $ScriptLogFilePath -Value $text
    ...
    Function Test-CCMSetup1 {
       ...
       if ($currentValue -ne $correctValue) {
            Set-ItemProperty -Path  'HKU:\S-1-5-18\Software\Microsoft\Windows\CurrentVersion\Explorer\User Shell Folders\' -Name 'AppData' -Value $correntValue
        }
    }
                ...
                foreach ($hotfix in $hotfixes) {
                    ...
                    if ($installedUpdates -like $kb) {
                        ...
                        if ($fix -eq "true") {
                            ...
                            If ((Test-Path C:\Temp\ClientHealth) -eq $false) {
                                New-Item -Path C:\Temp\ClientHealth -ItemType Directory | Out-Null
                            }
                
                            Copy-Item -Path $kbfullpath -Destination C:\Temp\ClientHealth
                            $install = "C:\Temp\ClientHealth\" +$hotfix
    ...
    Function Test-ConfigMgrClient {
        ...
        # Process the test result and determine if we reinstall the CM client
        if ($LocalDB -eq $true) {
        ...
        if (($CCMService.Status -eq "Stopped") -and ($LocalDB -eq $false)) {
            try {
                ...
                Out-LogFile -Xml $xml -Text "ConfigMgr Agent not running. Attempting to start it."
                $CCMService.Start()
            }
            catch {
                $Reinstall = $true
        ...
        if ( $reinstall -eq $true) {
            ...
            if ($localDB -eq $true) {
                ...
                $clientInstallProperties = $clientInstallProperties + " /forceinstall"
    ...
    Function Test-ClientSiteCode {
        ...
        if ($ClientSiteCode -like $currentSiteCode -Or $ClientSiteCode -eq "Auto") {
            ### Oliver: Set Auto Field on SiteCode XML Field so that Multplie Sitecodes are supported, badly.
    ...
    Function Start-Ccmeval {
        $task = "Configuration Manager Health Evaluation"
    ...
    Function Test-SoftwareMeteringPrepDriver {
        ...
        if (($content -match $error1) -or ($content -match $error2)) {
            ...
            if ($fix -eq "true") {
                ...
                $newContent = $content | Select-String -pattern $error1 -notmatch | Select-String -pattern $error2 -notmatch
                Stop-Service -Name CcmExec
                Out-File -FilePath $logfile -InputObject $newContent -Encoding utf8 -Force
                Start-Service -Name CcmExec
    ...
    Function Get-XMLConfigLoggingLocalPath {
        $obj = $Xml.Configuration.Log | Where-Object {$_.Name -like 'File'} | Select-Object -ExpandProperty 'LocalLogPath'
        Write-Output $obj
    }
    ...
    Function CleanUp {
        if ((Test-Path 'C:\Temp\ClientHealth' -ErrorAction SilentlyContinue) -eq $True) {
            Write-Verbose 'Cleaning up temporary files in C:\Temp\ClientHealth'
            Remove-Item 'C:\Temp\ClientHealth' -Recurse -Force | Out-Null
        }
    }
    ...
    $LogShare = Get-XMLConfigLoggingShare
    $LogLocal = Get-XMLConfigLoggingLocalPath
...
Process {
    ...
    if ((Test-ClientSiteCode -Log $Log) -eq $true) {$reinstall = $true}
